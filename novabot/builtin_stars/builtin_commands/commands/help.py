...
class HelpCommand:
    ...
    async def _query_novabot_notice(self):
        try:
            async with aiohttp.ClientSession(trust_env=True) as session:
                async with session.get(
                    "https://novabot.app/notice.json",
                    timeout=2,
                ) as resp:
    ...
    async def help(self, event: NovaMessageEvent) -> None:
        """查看帮助"""
        notice = ""
        try:
            notice = await self._query_novabot_notice()
        except BaseException:
            pass
