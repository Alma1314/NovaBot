...
from novabot.core.utils.novabot_path import get_novabot_path

from .zip_updator import ReleaseInfo, RepoZipUpdator


class NovaBotUpdator(RepoZipUpdator):
    """NovaBot 更新器，继承自 RepoZipUpdator 类
    该类用于处理 NovaBot 的更新操作
    ...
    def __init__(self, repo_mirror: str = "", verify: str | bool | None = None) -> None:
        ...
        self.MAIN_PATH = get_novabot_path()
        self.NOVABOT_RELEASE_API = "https://api.soulter.top/releases"
    ...
    def _build_frozen_reboot_args(self) -> list[str]:
        ...
        if not webui_dir:
            webui_dir = os.environ.get("NOVABOT_WEBUI_DIR")
    ...
    def _build_reboot_argv(self, executable: str) -> list[str]:
        if os.environ.get("NOVABOT_CLI") == "1":
            args = sys.argv[1:]
            return [executable, "-m", "novabot.cli.__main__", *args]
    ...
    async def check_update(
        ...
        return await super().check_update(
            url or self.NOVABOT_RELEASE_API,
            current_version or VERSION,
            consider_prerelease,
        )

    async def get_releases(self) -> list:
        return await self.fetch_release_info(self.NOVABOT_RELEASE_API)
    ...
    async def update(
        ...
        if version is None and not latest:
            raise ValueError("version must be provided when latest is False")

        update_data = await self.fetch_release_info(self.NOVABOT_RELEASE_API, latest)
        file_url = None

        if os.environ.get("NOVABOT_CLI") or os.environ.get("NOVABOT_LAUNCHER"):
            raise Exception(
                "Error: You are running NovaBot via CLI, please use `pip` or `uv tool upgrade` to update NovaBot."
            )  # 避免版本管理混乱
        ...
        try:
            ...
            logger.info("下载 NovaBot Core 更新文件完成，正在执行解压...")
