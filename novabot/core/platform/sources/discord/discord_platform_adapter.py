...
from novabot.api.message_components import File, Image, Plain
...
from novabot.core.star.filter.command import CommandFilter
from novabot.core.star.filter.command_group import CommandGroupFilter
from novabot.core.star.star import star_map
from novabot.core.star.star_handler import StarHandlerMetadata, star_handlers_registry
...
class DiscordPlatformAdapter(Platform):
    ...
    def _convert_message_to_abm(self, data: dict) -> NovaBotMessage:
        """将普通消息转换为 NovaBotMessage"""
    ...
    async def convert_message(self, data: dict) -> NovaBotMessage:
        """将平台消息转换成 NovaBotMessage"""
        # 由于 on_interaction 已被禁用，我们只处理普通消息
        return self._convert_message_to_abm(data)

    async def handle_msg(self, message: NovaBotMessage, followup_webhook=None) -> None:
        """处理消息"""
