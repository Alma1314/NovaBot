...
class AiocqhttpAdapter(Platform):
    ...
    async def convert_message(self, event: Event) -> NovaBotMessage | None:
    ...
    async def _convert_handle_request_event(self, event: Event) -> NovaBotMessage:
        """OneBot V11 请求类事件"""
        abm = NovaBotMessage()
    ...
    async def _convert_handle_notice_event(self, event: Event) -> NovaBotMessage:
        """OneBot V11 通知类事件"""
        abm = NovaBotMessage()
        abm.self_id = str(event.self_id)
        abm.sender = MessageMember(
