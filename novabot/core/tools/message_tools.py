...
from novabot.core.agent.tool import FunctionTool, ToolExecResult
from novabot.core.nova_agent_context import NovaAgentContext
...
from novabot.core.platform.message_session import MessageSession
from novabot.core.tools.computer_tools.util import check_admin_permission
from novabot.core.tools.registry import builtin_tool
from novabot.core.utils.novabot_path import get_novabot_temp_path


@builtin_tool
@dataclass
class SendMessageToUserTool(FunctionTool[NovaAgentContext]):
    ...
    async def _resolve_path_from_sandbox(
        self,
        context: ContextWrapper[NovaAgentContext],
        path: str,
        *,
        ...
        if not os.path.isabs(path):
            ...
            if unified_msg_origin:
                from novabot.core.tools.computer_tools.util import workspace_root
        ...
        try:
            ...
            if "_&exists_" in json.dumps(result):
                ...
                local_path = os.path.join(
                    get_novabot_temp_path(), f"sandbox_{uuid.uuid4().hex[:4]}_{name}"
                )
