...
from bulinbot.api.message_components import *
...
class AiocqhttpAdapter(Platform):
    ...
    async def convert_message(self, event: Event) -> BulinBotMessage | None:
    ...
    async def _convert_handle_request_event(self, event: Event) -> BulinBotMessage:
        """OneBot V11 请求类事件"""
        abm = BulinBotMessage()
    ...
    async def _convert_handle_notice_event(self, event: Event) -> BulinBotMessage:
        """OneBot V11 通知类事件"""
        abm = BulinBotMessage()
        abm.self_id = str(event.self_id)
        abm.sender = MessageMember(
