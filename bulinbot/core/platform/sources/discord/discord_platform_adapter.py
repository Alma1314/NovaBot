...
from bulinbot.api.message_components import File, Image, Plain
...
from bulinbot.core.star.star_handler import StarHandlerMetadata, star_handlers_registry
...
class DiscordPlatformAdapter(Platform):
    ...
    def _convert_message_to_abm(self, data: dict) -> BulinBotMessage:
        """将普通消息转换为 BulinBotMessage"""
    ...
    async def convert_message(self, data: dict) -> BulinBotMessage:
        """将平台消息转换成 BulinBotMessage"""
        # 由于 on_interaction 已被禁用，我们只处理普通消息
        return self._convert_message_to_abm(data)

    async def handle_msg(self, message: BulinBotMessage, followup_webhook=None) -> None:
        """处理消息"""
