...
from bulinbot.core.provider.func_tool_manager import PY_TO_JSON_TYPE, SUPPORTED_TYPES
...
def register_on_bulinbot_loaded(**kwargs):
    """当 BulinBot 加载完成时"""

    def decorator(awaitable):
        _ = get_handler_or_create(awaitable, EventType.OnBulinBotLoadedEvent, **kwargs)
        return awaitable
...
def register_on_waiting_llm_request(**kwargs):
    ...
    async def on_waiting_llm(self, event: BulinMessageEvent) -> None:
...
def register_on_llm_request(**kwargs):
    ...
    @on_llm_request()
    async def test(self, event: BulinMessageEvent, req: ProviderRequest) -> None:
...
def register_on_llm_response(**kwargs):
    ...
    @on_llm_response()
    async def test(self, event: BulinMessageEvent, response: LLMResponse) -> None:
        ...
    ```
...
def register_on_using_llm_tool(**kwargs):
    ...
    @on_using_llm_tool()
    async def test(self, event: BulinMessageEvent, tool: FunctionTool, tool_args: dict | None) -> None:
        ...
    ```
...
def register_on_llm_tool_respond(**kwargs):
    ...
    @on_llm_tool_respond()
    async def test(self, event: BulinMessageEvent, tool: FunctionTool, tool_args: dict | None, tool_result: CallToolResult | None) -> None:
        ...
    ```
...
def register_llm_tool(name: str | None = None, **kwargs):
    ...
    请务必按照以下格式编写一个工具（包括函数注释，BulinBot 会尝试解析该函数注释）

    ```
    @llm_tool(name="get_weather") # 如果 name 不填，将使用函数名
    async def get_weather(event: BulinMessageEvent, location: str):
        \'\'\'获取天气信息。
