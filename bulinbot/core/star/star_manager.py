...
from bulinbot.core import (
    DependencyConflictError,
    logger,
    pip_installer,
    sp,
)
from bulinbot.core.agent.handoff import FunctionTool, HandoffTool
from bulinbot.core.config.bulinbot_config import BulinBotConfig
from bulinbot.core.config.default import VERSION
from bulinbot.core.platform.register import unregister_platform_adapters_by_module
...
from bulinbot.core.utils.io import remove_dir
from bulinbot.core.utils.metrics import Metric
...
try:
    ...
    if os.getenv("BULINBOT_RELOAD", "0") == "1":
        logger.warning("未安装 watchfiles，无法实现插件的热重载。")


class PluginVersionIncompatibleError(Exception):
    """Raised when plugin bulinbot_version is incompatible with current BulinBot."""
...
def _temporary_filtered_requirements_file(
    *,
    ...
    temp_dir = get_bulinbot_temp_path()
...
class PluginManager:
    def __init__(self, context: Context, config: BulinBotConfig) -> None:
        ...
        self.config = config
        self.plugin_store_path = get_bulinbot_plugin_path()
        """存储插件的路径。即 data/plugins"""
        self.plugin_config_path = get_bulinbot_config_path()
        """存储插件配置的路径。data/config"""
        self.reserved_plugin_path = os.path.join(
            get_bulinbot_path(), "bulinbot", "builtin_stars"
        )
        """保留插件的路径。在 bulinbot/builtin_stars 目录下"""
    ...
    def _load_plugin_metadata(plugin_path: str, plugin_obj=None) -> StarMetadata | None:
        ...
        Notes: 旧版本 BulinBot 插件可能使用的是 info() 函数来获取元数据。
        """
        metadata = None
        ...
        if isinstance(metadata, dict):
            ...
            metadata = StarMetadata(
                ...
                bulinbot_version=(
                    metadata["bulinbot_version"]
                    if isinstance(metadata.get("bulinbot_version"), str)
                    else None
                ),
    ...
    def _load_plugin_i18n(plugin_path: str) -> dict[str, dict]:
        ...
        i18n_dir = plugin_root / ".bulinbot-plugin" / "i18n"
    ...
    @staticmethod
    def _validate_bulinbot_version_specifier(
        ...
        try:
            ...
            return (
                False,
                f"Invalid current BulinBot version: {VERSION}. Cannot check plugin version range.",
            )

        if not specifier.contains(current_version, prereleases=True):
            return (
                False,
                f"BulinBot {VERSION} does not satisfy plugin bulinbot_version: {normalized_spec}",
            )
        return True, None
    ...
    def _get_plugin_related_modules(
        ...
        """
        prefix = "bulinbot.builtin_stars." if is_reserved else "data.plugins."
    ...
    def _purge_modules(
        ...
        Args:
            module_patterns: 要移除的模块名模式列表（例如 ["data.plugins", "bulinbot.builtin_stars"]）
    ...
    def _cleanup_plugin_state(self, dir_name: str, is_reserved: bool = False) -> None:
        plugin_root_name = "bulinbot.builtin_stars." if is_reserved else "data.plugins."
                ...
                record.update(
                    {
                        ...
                        "bulinbot_version": metadata.bulinbot_version,
                    }
                )
    ...
    def _rebuild_failed_plugin_info(self) -> None:
        ...
        for dir_name, info in self.failed_plugin_dict.items():
            if isinstance(info, dict):
                ...
                version = info.get("version") or info.get("bulinbot_version")
                if version:
                    lines.append(
        ...
        for plugin_module in plugin_modules:
            try:
                ...
                reserved = plugin_module.get(
                    "reserved",
                    False,
                )  # 是否是保留插件。目前在 bulinbot/builtin_stars 目录下的都是保留插件。保留插件不可以卸载。
                plugin_dir_path = (
                ...
                path = "data.plugins." if not reserved else "bulinbot.builtin_stars."
                ...
                if path in star_map:
                    ...
                    try:
                        ...
                        if metadata_yaml:
                            ...
                            metadata.bulinbot_version = metadata_yaml.bulinbot_version
                    ...
                    if not ignore_version_check:
                        is_valid, error_message = (
                            self._validate_bulinbot_version_specifier(
                                metadata.bulinbot_version,
                            )
                        )
                        if not is_valid:
                            raise PluginVersionIncompatibleError(
                                error_message
                                or "The plugin is not compatible with the current BulinBot version."
                            )
                    ...
                    if not classes:
                        raise Exception(
                            ...
                            "请确认插件主类继承 bulinbot.api.star.Star，或类名以 Plugin 结尾 / 命名为 Main。",
                        )
    ...
    async def uninstall_plugin(
        ...
        async with self._pm_lock:
            ...
            if not plugin:
                raise Exception("插件不存在。")
            if plugin.reserved:
                raise Exception("该插件是 BulinBot 保留插件，无法卸载。")
    ...
    async def _unbind_plugin(self, plugin_name: str, plugin_module_path: str) -> None:
        ...
        for func_tool in llm_tools.func_list:
            ...
            if (
                ...
                and not mp.endswith(("bulinbot.builtin_stars", "data.plugins"))
            ):
    ...
    async def update_plugin(
        ...
        if not plugin:
            raise Exception("插件不存在。")
        if plugin.reserved:
            raise Exception("该插件是 BulinBot 保留插件，无法更新。")
