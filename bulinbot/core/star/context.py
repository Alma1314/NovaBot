...
from bulinbot.core.platform import Platform
from bulinbot.core.platform.bulin_message_event import BulinMessageEvent, MessageSesion
from bulinbot.core.platform_message_history_mgr import PlatformMessageHistoryManager
from bulinbot.core.provider.entities import LLMResponse, ProviderRequest, ProviderType
from bulinbot.core.provider.func_tool_manager import FunctionTool, FunctionToolManager
from bulinbot.core.provider.manager import ProviderManager
from bulinbot.core.provider.provider import (
    EmbeddingProvider,
    Provider,
    RerankProvider,
    STTProvider,
...
from bulinbot.core.utils.bulinbot_path import get_bulinbot_system_tmp_path
...
logger = logging.getLogger("bulinbot")

if TYPE_CHECKING:
    from bulinbot.core.cron.manager import CronJobManager
...
class Context:
    ...
    def __init__(
        ...
        bulinbot_config_mgr: BulinBotConfigManager,
        ...
        """人格角色设定管理器"""
        self.bulinbot_config_mgr = bulinbot_config_mgr
        """配置文件管理器(非webui)"""
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            **kwargs: Additional keyword arguments. The kwargs will not be passed to the LLM directly for now, but can include:
                ...
                agent_hooks: BaseAgentRunHooks[BulinAgentContext] - hooks to run during agent execution
                agent_context: BulinAgentContext - context to use for the agent
        ...
        from bulinbot.core.bulin_agent_context import (
            AgentContextWrapper,
            BulinAgentContext,
        )
        ...
        agent_hooks = kwargs.get("agent_hooks") or BaseAgentRunHooks[BulinAgentContext]()
        ...
        if agent_context is None:
            agent_context = BulinAgentContext(
                context=self,
                event=event,
            )
        ...
        if request.func_tool and request.func_tool.get_tool("bulinbot_file_read_tool"):
            other_kwargs.setdefault(
                "tool_result_overflow_dir", get_bulinbot_system_tmp_path()
            )
            other_kwargs.setdefault(
                "read_tool", request.func_tool.get_tool("bulinbot_file_read_tool")
            )
    ...
    def get_config(self, umo: str | None = None) -> BulinBotConfig:
        """获取 BulinBot 的配置。
        ...
        if not umo:
            # 使用默认配置
            return self._config
        return self.bulinbot_config_mgr.get_conf(umo)
    ...
    """
    以下的方法已经不推荐使用。请从 BulinBot 文档查看更好的注册方式。
    """
    ...
    def get_platform(self, platform_type: PlatformAdapterType | str) -> Platform | None:
        ...
        Note:
            该方法已经过时，请使用 get_platform_inst 方法。(>= BulinBot v4.0.0)
        """
        for platform in self.platform_manager.platform_insts:
    ...
    def register_llm_tool(
        ...
        Note:
            异步处理函数会接收到额外的关键词参数：event: BulinMessageEvent, context: Context。
            该方法已弃用，请使用新的注册方式。
        """
