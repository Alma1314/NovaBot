...
from bulinbot.core import logger, sp
...
from bulinbot.core.tools.registry import builtin_tool
...
class _KeyRotator:
    ...
    async def get(self, provider_settings: dict) -> str:
        ...
        if not keys:
            raise ValueError(
                f"Error: {self.provider_name} API key is not configured in BulinBot."
            )
...
async def _baidu_search(
    ...
    if not api_key:
        raise ValueError("Error: Baidu AI Search API key is not configured in BulinBot.")
...
@pydantic_dataclass
class TavilyWebSearchTool(FunctionTool[BulinAgentContext]):
    ...
    async def call(self, context, **kwargs) -> ToolExecResult:
        ...
        if not provider_settings.get("websearch_tavily_key", []):
            return "Error: Tavily API key is not configured in BulinBot."
...
@builtin_tool(config=_TAVILY_WEB_SEARCH_TOOL_CONFIG)
@pydantic_dataclass
class TavilyExtractWebPageTool(FunctionTool[BulinAgentContext]):
...
@pydantic_dataclass
class BochaWebSearchTool(FunctionTool[BulinAgentContext]):
    ...
    async def call(self, context, **kwargs) -> ToolExecResult:
        ...
        if not provider_settings.get("websearch_bocha_key", []):
            return "Error: BoCha API key is not configured in BulinBot."
...
@pydantic_dataclass
class BraveWebSearchTool(FunctionTool[BulinAgentContext]):
    ...
    async def call(self, context, **kwargs) -> ToolExecResult:
        ...
        if not provider_settings.get("websearch_brave_key", []):
            return "Error: Brave API key is not configured in BulinBot."
...
@pydantic_dataclass
class FirecrawlWebSearchTool(FunctionTool[BulinAgentContext]):
    ...
    async def call(self, context, **kwargs) -> ToolExecResult:
        ...
        if not provider_settings.get("websearch_firecrawl_key", []):
            return "Error: Firecrawl API key is not configured in BulinBot."
...
@builtin_tool(config=_FIRECRAWL_WEB_SEARCH_TOOL_CONFIG)
@pydantic_dataclass
class FirecrawlExtractWebPageTool(FunctionTool[BulinAgentContext]):
...
@pydantic_dataclass
class BaiduWebSearchTool(FunctionTool[BulinAgentContext]):
    ...
    async def call(self, context, **kwargs) -> ToolExecResult:
        ...
        if not provider_settings.get("websearch_baidu_app_builder_key", ""):
            return "Error: Baidu AI Search API key is not configured in BulinBot."
