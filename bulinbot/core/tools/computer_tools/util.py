...
from bulinbot.core.utils.bulinbot_path import get_bulinbot_workspaces_path
...
def is_local_runtime(context: ContextWrapper[BulinAgentContext]) -> bool:
...
def check_admin_permission(
    context: ContextWrapper[BulinAgentContext], operation_name: str
) -> str | None:
    cfg = context.context.context.get_config(
        umo=context.context.event.unified_msg_origin
    )
    ...
    if require_admin and context.context.event.role != "admin":
        return (
            ...
            "Tell user to set admins in `BulinBot WebUI -> Config -> General Config` by adding their user ID to the admins list if they need this feature. "
