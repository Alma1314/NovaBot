...
def _build_background_output_path(*, local_runtime: bool) -> str:
    file_name = f"bulinbot_shell_stdout_{uuid.uuid4().hex[:8]}.log"
    if local_runtime:
        output_dir = Path(get_bulinbot_system_tmp_path()) / "shell"
...
class ExecuteShellTool(FunctionTool):
    name: str = "bulinbot_execute_shell"
        ...
        try:
            ...
            if stdout_file:
                result["stdout"] = (
                    ...
                    f"written to `{stdout_file}`. Use bulinbot_file_read_tool to read it."
                )
