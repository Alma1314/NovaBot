...
- Local runtime exposes `bulinbot_read_file_tool`, `bulinbot_file_write_tool`,
  `bulinbot_file_edit_tool`, and `bulinbot_grep_tool`.
- Sandbox runtime exposes `bulinbot_upload_file`, `bulinbot_download_file`,
  `bulinbot_read_file_tool`, `bulinbot_file_write_tool`,
  `bulinbot_file_edit_tool`, and `bulinbot_grep_tool`.
...
- Member + local: read/grep are restricted to `data/skills`,
  ...
  `data/workspaces/{normalized_umo}`, and `/tmp/.bulinbot`; write/edit are
...
from bulinbot.core.computer.file_read_utils import read_file_tool_result
from bulinbot.core.message.components import File, Image
...
def get_bulinbot_workspaces_path() -> str:
    ...
    return computer_util.get_bulinbot_workspaces_path()


def _workspace_root(umo: str) -> Path:
    """Workspace root that follows both util-level and fs-level getter monkeypatches."""
    normalized_umo = normalize_umo_for_workspace(umo)
    return (Path(get_bulinbot_workspaces_path()) / normalized_umo).resolve(strict=False)


def _plugin_skill_roots() -> tuple[Path, ...]:
    plugins_root = Path(get_bulinbot_plugin_path())
...
def _read_allowed_roots(umo: str) -> tuple[Path, ...]:
    ...
    return (
        Path(get_bulinbot_skills_path()).resolve(strict=False),
        *_plugin_skill_roots(),
        _workspace_root(umo),
        Path(get_bulinbot_system_tmp_path()).resolve(strict=False),
        Path(get_bulinbot_temp_path()).resolve(strict=False),
    )
...
def _is_restricted_env(context: ContextWrapper[BulinAgentContext]) -> bool:
...
class FileReadTool(FunctionTool):
    name: str = "bulinbot_file_read_tool"
    ...
    async def call(
        self,
        context: ContextWrapper[BulinAgentContext],
        path: str,
...
class FileWriteTool(FunctionTool):
    name: str = "bulinbot_file_write_tool"
...
class FileEditTool(FunctionTool):
    name: str = "bulinbot_file_edit_tool"
...
class FileDownloadTool(FunctionTool):
    name: str = "bulinbot_download_file"
    description: str = (
    ...
    async def call(
        ...
        try:
            ...
            local_path = os.path.join(
                get_bulinbot_temp_path(), f"sandbox_{uuid.uuid4().hex[:4]}_{name}"
            )
