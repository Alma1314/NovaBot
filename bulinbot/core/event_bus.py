...
from bulinbot.core.pipeline.scheduler import PipelineScheduler

from .platform import BulinMessageEvent
...
class EventBus:
    ...
    async def dispatch(self) -> None:
        while True:
            event: BulinMessageEvent = await self.event_queue.get()
            conf_info = self.bulinbot_config_mgr.get_conf_info(event.unified_msg_origin)
    ...
    def _print_event(self, event: BulinMessageEvent, conf_name: str) -> None:
        """用于记录事件信息

        Args:
            event (BulinMessageEvent): 事件对象
