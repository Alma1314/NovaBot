...
from bulinbot.core.agent.mcp_client import MCPClient, MCPTool
from bulinbot.core.agent.tool import FunctionTool, ToolSet
from bulinbot.core.tools.registry import (
...
MCP_INIT_TIMEOUT_ENV = "BULINBOT_MCP_INIT_TIMEOUT"
ENABLE_MCP_TIMEOUT_ENV = "BULINBOT_MCP_ENABLE_TIMEOUT"
...
class FunctionToolManager:
    def __init__(self) -> None:
        ...
        """All tools include mcp tools and plugin tools, except bulinbot builtin tools."""
        self.builtin_func_list: dict[type[FuncTool], FuncTool] = {}
        """All bulinbot builtin tools, keyed by their class. Values are instantiated tool objects, created on demand."""
    ...
    async def init_mcp_clients(
        ...
        Timeout behavior:
        - 初始化超时使用环境变量 BULINBOT_MCP_INIT_TIMEOUT 或默认值。
        - 动态启用超时使用 BULINBOT_MCP_ENABLE_TIMEOUT（独立于初始化超时）。
        """
    ...
    async def enable_mcp_server(
        ...
        Args:
            ...
            timeout: Timeout in seconds for initialization.
                Uses BULINBOT_MCP_ENABLE_TIMEOUT by default (separate from init timeout).
