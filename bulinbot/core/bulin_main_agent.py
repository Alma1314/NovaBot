...
from bulinbot.core import logger
...
from bulinbot.core.agent.tool import ToolSet
from bulinbot.core.bulin_agent_context import AgentContextWrapper, BulinAgentContext
from bulinbot.core.bulin_agent_hooks import MAIN_AGENT_HOOKS
from bulinbot.core.bulin_agent_run_util import AgentRunner
from bulinbot.core.bulin_agent_tool_exec import FunctionToolExecutor
from bulinbot.core.bulin_main_agent_resources import (
...
from bulinbot.core.message.components import File, Image, Record, Reply, Video
from bulinbot.core.persona_error_reply import (
...
from bulinbot.core.provider.register import llm_tools
from bulinbot.core.skills.skill_manager import (
    SkillInfo,
    SkillManager,
    build_skills_prompt,
)
from bulinbot.core.star.context import Context
from bulinbot.core.star.star import star_registry
from bulinbot.core.star.star_handler import star_map
from bulinbot.core.tools.computer_tools import (
...
from bulinbot.core.tools.knowledge_base_tools import (
...
from bulinbot.core.utils.bulinbot_path import (
    get_bulinbot_system_tmp_path,
    get_bulinbot_workspaces_path,
)
from bulinbot.core.utils.file_extract import extract_file_moonshotai
from bulinbot.core.utils.llm_metadata import LLM_METADATAS
from bulinbot.core.utils.media_utils import (
    IMAGE_COMPRESS_DEFAULT_MAX_SIZE,
    IMAGE_COMPRESS_DEFAULT_QUALITY,
    compress_image,
)
from bulinbot.core.utils.quoted_message.settings import (
...
from bulinbot.core.utils.string_utils import normalize_and_dedupe_strings
...
def _set_llm_error_message(event: BulinMessageEvent, message: str) -> None:
    event.set_extra(LLM_ERROR_MESSAGE_EXTRA_KEY, message)


def _select_provider(
    event: BulinMessageEvent, plugin_context: Context
) -> Provider | None:
...
def _get_workspace_path_for_umo(umo: str) -> Path:
    ...
    return Path(get_bulinbot_workspaces_path()) / normalized_umo
        ...
        if skills:
            ...
            if runtime == "none":
                req.system_prompt += (
                    ...
                    "If you need to use these capabilities, ask the user to enable Computer Use in the BulinBot WebUI -> Config."
                )
...
def _plugin_tool_fix(event: BulinMessageEvent, req: ProviderRequest) -> None:
...
async def _handle_webchat(
    event: BulinMessageEvent, req: ProviderRequest, prov: Provider
) -> None:
    from bulinbot.core import db_helper
...
def _apply_sandbox_tools(
    ...
    if booter == "shipyard_neo":
        ...
        req.system_prompt += (
            ...
            "Preferred sequence:\n"
            "1) Use `bulinbot_create_skill_payload` to store canonical payload content and get `payload_ref`.\n"
            "2) Use `bulinbot_create_skill_candidate` with `skill_key` + `source_execution_ids` (and optional `payload_ref`) to create a candidate.\n"
            "3) Use `bulinbot_promote_skill_candidate` to release: `stage=canary` for trial; `stage=stable` for production.\n"
        ...
        from bulinbot.core.computer.computer_client import session_booter
    ...
    if booter == "cua":
        req.system_prompt += (
            "\n[CUA Desktop Control]\n"
            "Use `bulinbot_execute_shell` with `background=true` to launch GUI apps. "
            'Use Firefox for browser tasks, for example `firefox "https://example.com"`. '
            "After each visible step, call `bulinbot_cua_screenshot` with "
            ...
            "`bulinbot_cua_mouse_click` for coordinates and `bulinbot_cua_keyboard_type` "
...
def _get_compress_provider(
    config: MainAgentBuildConfig,
    plugin_context: Context,
    event: BulinMessageEvent | None = None,
) -> Provider | None:
...
async def build_main_agent(
    *,
    ...
    agent_runner = AgentRunner()
    bulin_agent_ctx = BulinAgentContext(
        context=plugin_context,
        event=event,
    )
    ...
    reset_coro = agent_runner.reset(
        ...
        tool_result_overflow_dir=(
            get_bulinbot_system_tmp_path()
            if req.func_tool and req.func_tool.get_tool("bulinbot_file_read_tool")
            else None
        ),
        read_tool=(
            req.func_tool.get_tool("bulinbot_file_read_tool") if req.func_tool else None
        ),
    )
