...
class BulinBotError(Exception):
    """Base exception for all BulinBot errors.
    错误的基类"""


class ProviderNotFoundError(BulinBotError):
    """Raised when a specified provider is not found.
    查不到指定的 LLM Provider 时抛出"""


class EmptyModelOutputError(BulinBotError):
    """Raised when the model response contains no usable assistant output.
    LLM 返回空响应时抛出"""


class KnowledgeBaseUploadError(BulinBotError):
