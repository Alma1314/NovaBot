...
from bulinbot.core.utils.bulinbot_path import get_bulinbot_path

from .zip_updator import ReleaseInfo, RepoZipUpdator


class BulinBotUpdator(RepoZipUpdator):
    """BulinBot 更新器，继承自 RepoZipUpdator 类
    该类用于处理 BulinBot 的更新操作
    ...
    def __init__(self, repo_mirror: str = "", verify: str | bool | None = None) -> None:
        ...
        self.MAIN_PATH = get_bulinbot_path()
        self.BULINBOT_RELEASE_API = "https://api.soulter.top/releases"
    ...
    def _build_frozen_reboot_args(self) -> list[str]:
        ...
        if not webui_dir:
            webui_dir = os.environ.get("BULINBOT_WEBUI_DIR")
    ...
    def _build_reboot_argv(self, executable: str) -> list[str]:
        if os.environ.get("BULINBOT_CLI") == "1":
            args = sys.argv[1:]
            return [executable, "-m", "bulinbot.cli.__main__", *args]
    ...
    async def check_update(
        ...
        return await super().check_update(
            self.BULINBOT_RELEASE_API,
            VERSION,
            consider_prerelease,
        )

    async def get_releases(self) -> list:
        return await self.fetch_release_info(self.BULINBOT_RELEASE_API)
    ...
    async def update(
        ...
        update_data = await self.fetch_release_info(self.BULINBOT_RELEASE_API, latest)
        file_url = None

        if os.environ.get("BULINBOT_CLI") or os.environ.get("BULINBOT_LAUNCHER"):
            raise Exception(
                "Error: You are running BulinBot via CLI, please use `pip` or `uv tool upgrade` to update BulinBot."
            )  # 避免版本管理混乱
        ...
        if latest:
            ...
            file_url = f"https://github.com/BulinBotDevs/BulinBot/archive/{version}.zip"
        logger.info(f"准备更新至指定版本的 BulinBot Core: {version}")
        ...
        try:
            ...
            logger.info("下载 BulinBot Core 更新文件完成，正在执行解压...")
