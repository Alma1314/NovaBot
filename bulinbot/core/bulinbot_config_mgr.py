...
from bulinbot.core import BulinBotConfig, logger
from bulinbot.core.config.bulinbot_config import BULINBOT_CONFIG_PATH
...
from bulinbot.core.utils.bulinbot_path import get_bulinbot_config_path
...
DEFAULT_CONFIG_CONF_INFO = ConfInfo(
    id="default",
    name="default",
    path=BULINBOT_CONFIG_PATH,
)


class BulinBotConfigManager:
    """A class to manage the system configuration of BulinBot, aka ACM"""

    def __init__(
        self,
        default_config: BulinBotConfig,
        ucr: UmopConfigRouter,
        sp: SharedPreferences,
    ) -> None:
        self.sp = sp
        self.ucr = ucr
        self.confs: dict[str, BulinBotConfig] = {}
        """uuid / "default" -> BulinBotConfig"""
    ...
    def _load_all_configs(self) -> None:
        ...
        for uuid_, meta in abconf_data.items():
            filename = meta["path"]
            conf_path = os.path.join(get_bulinbot_config_path(), filename)
            if os.path.exists(conf_path):
                conf = BulinBotConfig(config_path=conf_path)
    ...
    def get_conf(self, umo: str | MessageSession | None) -> BulinBotConfig:
    ...
    @property
    def default_conf(self) -> BulinBotConfig:
        """获取默认配置文件"""
        return self.confs["default"]
    ...
    def create_conf(
        ...
        conf_path = os.path.join(get_bulinbot_config_path(), conf_file_name)
        conf = BulinBotConfig(config_path=conf_path, default_config=config)
        conf.save_config()
