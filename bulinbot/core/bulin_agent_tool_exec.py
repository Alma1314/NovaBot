...
from bulinbot.core.cron.events import CronMessageEvent
...
from bulinbot.core.platform.message_session import MessageSession
from bulinbot.core.provider.entites import ProviderRequest
...
from bulinbot.core.utils.bulinbot_path import get_bulinbot_temp_path
from bulinbot.core.utils.history_saver import persist_agent_history
from bulinbot.core.utils.image_ref_utils import is_supported_image_ref
from bulinbot.core.utils.string_utils import normalize_and_dedupe_strings


class FunctionToolExecutor(BaseFunctionToolExecutor[BulinAgentContext]):
    @classmethod
    ...
    async def _collect_image_urls_from_message(
        cls, run_context: ContextWrapper[BulinAgentContext]
    ) -> list[str]:
        urls: list[str] = []
    ...
    async def _collect_handoff_image_urls(
        cls,
        run_context: ContextWrapper[BulinAgentContext],
        ...
        extensionless_local_roots = (get_bulinbot_temp_path(),)
        sanitized = [
            item
    ...
    async def execute(cls, tool, run_context, **tool_args):
        ...
        Args:
            event (BulinMessageEvent): 事件对象, 当 origin 为 local 时必须提供。
            **kwargs: 函数调用的参数。
    ...
    async def _wake_main_agent_for_background_result(
        ...
        *,
        ...
        from bulinbot.core.bulin_main_agent import (
