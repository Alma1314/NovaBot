"""BulinBot 核心生命周期管理类, 负责管理 BulinBot 的启动、停止、重启等操作.
...
from bulinbot.api import logger, sp
from bulinbot.core import LogBroker, LogManager
from bulinbot.core.bulinbot_config_mgr import BulinBotConfigManager
from bulinbot.core.config.default import VERSION
from bulinbot.core.conversation_mgr import ConversationManager
from bulinbot.core.cron import CronJobManager
from bulinbot.core.db import BaseDatabase
from bulinbot.core.knowledge_base.kb_mgr import KnowledgeBaseManager
from bulinbot.core.persona_mgr import PersonaManager
from bulinbot.core.pipeline.scheduler import PipelineContext, PipelineScheduler
from bulinbot.core.platform.manager import PlatformManager
from bulinbot.core.platform_message_history_mgr import PlatformMessageHistoryManager
from bulinbot.core.provider.manager import ProviderManager
from bulinbot.core.star.context import Context
from bulinbot.core.star.star_handler import EventType, star_handlers_registry, star_map
from bulinbot.core.star.star_manager import PluginManager
from bulinbot.core.subagent_orchestrator import SubAgentOrchestrator
from bulinbot.core.umop_config_router import UmopConfigRouter
from bulinbot.core.updator import BulinBotUpdator
from bulinbot.core.utils.llm_metadata import update_llm_metadata
from bulinbot.core.utils.migra_helper import migra
from bulinbot.core.utils.temp_dir_cleaner import TempDirCleaner

from . import bulinbot_config, html_renderer
from .event_bus import EventBus


class BulinBotCoreLifecycle:
    """BulinBot 核心生命周期管理类, 负责管理 BulinBot 的启动、停止、重启等操作.
    ...
    def __init__(self, log_broker: LogBroker, db: BaseDatabase) -> None:
        ...
        self.bulinbot_config = bulinbot_config  # 初始化配置
        self.db = db  # 初始化数据库
    ...
    async def _init_or_reload_subagent_orchestrator(self) -> None:
        ...
        try:
            ...
            await self.subagent_orchestrator.reload_from_config(
                self.bulinbot_config.get("subagent_orchestrator", {}),
            )
    ...
    def _warn_about_unset_default_chat_provider(self) -> None:
        ...
        if not default_id:
            ...
            logger.warning(
                ...
                "BulinBot will use `%s` as the startup fallback chat provider. "
        ...
        if not found:
            ...
            logger.warning(
                ...
                "BulinBot will use `%s` as the fallback chat provider. "
    ...
    async def initialize(self) -> None:
        """初始化 BulinBot 核心生命周期管理类.

        负责初始化各个组件, 包括 ProviderManager、PlatformManager、ConversationManager、PluginManager、PipelineScheduler、EventBus、BulinBotUpdator等。
        """
        # 初始化日志代理
        logger.info("BulinBot v" + VERSION)
        if os.environ.get("TESTING", ""):
            LogManager.configure_logger(
                logger, self.bulinbot_config, override_level="DEBUG"
            )
            LogManager.configure_trace_logger(self.bulinbot_config)
        else:
            LogManager.configure_logger(logger, self.bulinbot_config)
            LogManager.configure_trace_logger(self.bulinbot_config)
        ...
        # 初始化 BulinBot 配置管理器
        self.bulinbot_config_mgr = BulinBotConfigManager(
            default_config=self.bulinbot_config,
            ucr=self.umop_config_router,
            sp=sp,
        )
        self.temp_dir_cleaner = TempDirCleaner(
            max_size_getter=lambda: self.bulinbot_config_mgr.default_conf.get(
        ...
        try:
            await migra(
                self.db,
                self.bulinbot_config_mgr,
                self.umop_config_router,
                self.bulinbot_config_mgr,
            )
        except Exception as e:
            logger.error(f"BulinBot migration failed: {e!s}")
        ...
        # 初始化人格管理器
        self.persona_mgr = PersonaManager(self.db, self.bulinbot_config_mgr)
        ...
        # 初始化平台管理器
        self.platform_manager = PlatformManager(self.bulinbot_config, self.event_queue)
        ...
        # 初始化插件管理器
        self.plugin_manager = PluginManager(self.star_context, self.bulinbot_config)
        ...
        # 初始化更新器
        self.bulinbot_updator = BulinBotUpdator()
    ...
    async def start(self) -> None:
        """启动 BulinBot 核心生命周期管理类.

        用load加载事件总线和任务并初始化, 执行启动完成事件钩子
        """
        self._load()
        logger.info("BulinBot started.")

        # 执行启动完成事件钩子
        handlers = star_handlers_registry.get_handlers_by_event_type(
            EventType.OnBulinBotLoadedEvent,
        )
        for handler in handlers:
            try:
                logger.info(
                    f"hook(on_bulinbot_loaded) -> {star_map[handler.handler_module_path].name} - {handler.handler_name}",
                )
    ...
    async def stop(self) -> None:
        """停止 BulinBot 核心生命周期管理类, 取消所有当前任务并终止各个管理器."""
        if self.temp_dir_cleaner:
    ...
    async def restart(self) -> None:
        """重启 BulinBot 核心生命周期管理类, 终止各个管理器并重新加载平台实例"""
        await self.provider_manager.terminate()
        await self.platform_manager.terminate()
        await self.kb_manager.terminate()
        self.dashboard_shutdown_event.set()
        threading.Thread(
            target=self.bulinbot_updator._reboot,
            name="restart",
            daemon=True,
        ).start()
    ...
    async def load_pipeline_scheduler(self) -> dict[str, PipelineScheduler]:
        ...
        """
        mapping = {}
        for conf_id, ab_config in self.bulinbot_config_mgr.confs.items():
    ...
    async def reload_pipeline_scheduler(self, conf_id: str) -> None:
        ...
        """
        ab_config = self.bulinbot_config_mgr.confs.get(conf_id)
        if not ab_config:
