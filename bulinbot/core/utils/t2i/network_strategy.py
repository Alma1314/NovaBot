...
from bulinbot.core.utils.http_ssl import build_tls_connector
from bulinbot.core.utils.io import download_image_by_url
from bulinbot.core.utils.t2i.template_manager import TemplateManager

from . import RenderStrategy

BULINBOT_T2I_DEFAULT_ENDPOINT = "https://t2i.soulter.top/text2img"
SHIKI_RUNTIME_SCRIPT_ID = "bulinbot-t2i-shiki-runtime"
...
class NetworkRenderStrategy(RenderStrategy):
    def __init__(self, base_url: str | None = None) -> None:
        super().__init__()
        if not base_url:
            self.BASE_RENDER_URL = BULINBOT_T2I_DEFAULT_ENDPOINT
        else:
    ...
    async def initialize(self) -> None:
        if self.BASE_RENDER_URL == BULINBOT_T2I_DEFAULT_ENDPOINT:
        ...
        try:
            async with aiohttp.ClientSession(
                ...
                async with session.get(
                    "https://api.soulter.top/bulinbot/t2i-endpoints",
                ) as resp:
                    if resp.status == 200:
