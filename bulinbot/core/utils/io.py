...
from .bulinbot_path import get_bulinbot_data_path, get_bulinbot_path, get_bulinbot_temp_path
...
def get_bundled_dashboard_dist_path() -> Path:
    return Path(get_bulinbot_path()) / "bulinbot" / "dashboard" / "dist"
...
async def get_dashboard_version():
    ...
    dist_dir = os.path.join(get_bulinbot_data_path(), "dist")
...
async def download_dashboard(
    ...
    """下载管理面板文件"""
    if path is None:
        zip_path = Path(get_bulinbot_data_path()).absolute() / "dashboard.zip"
    else:
        zip_path = Path(path).absolute()

    if latest or len(str(version)) != 40:
        ver_name = "latest" if latest else version
        dashboard_release_url = f"https://bulinbot-registry.soulter.top/download/bulinbot-dashboard/{ver_name}/dist.zip"
        logger.info(
            f"Downloading BulinBot WebUI from {dashboard_release_url}",
        )
        try:
            await download_file(
            ...
            if latest:
                ...
                async with aiohttp.ClientSession(
                    ...
                    async with session.get(
                        "https://api.github.com/repos/BulinBotDevs/BulinBot/releases/latest",
                        timeout=30,
                ...
                tag = version
            dashboard_release_url = f"https://github.com/BulinBotDevs/BulinBot/releases/download/{tag}/BulinBot-{tag}-dashboard.zip"
            if proxy:
        ...
        url = f"https://github.com/BulinBotDevs/bulinbot-release-harbour/releases/download/release-{version}/dist.zip"
        logger.info(f"Downloading BulinBot WebUI from {url}")
        if proxy:
            url = f"{proxy}/{url}"
        await download_file(
            url,
