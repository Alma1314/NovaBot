...
class ActiveEventRegistry:
    ...
    def __init__(self) -> None:
        self._events: dict[str, set[BulinMessageEvent]] = defaultdict(set)

    def register(self, event: BulinMessageEvent) -> None:
        self._events[event.unified_msg_origin].add(event)

    def unregister(self, event: BulinMessageEvent) -> None:
    ...
    def stop_all(
        self,
        umo: str,
        exclude: BulinMessageEvent | None = None,
    ) -> int:
        """终止指定 UMO 的所有活跃事件。
