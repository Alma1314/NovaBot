...
class SessionWaiter:
    def __init__(
        ...
        self.handler: (
            Callable[[SessionController, BulinMessageEvent], Awaitable[Any]] | None
        ) = None  # 处理函数
    ...
    async def register_wait(
        self,
        handler: Callable[[SessionController, BulinMessageEvent], Awaitable[Any]],
        timeout: int = 30,
    ) -> Any:
        """等待外部输入并处理"""
    ...
    @classmethod
    async def trigger(cls, session_id: str, event: BulinMessageEvent) -> None:
        """外部输入触发会话处理"""
...
def session_waiter(timeout: int = 30, record_history_chains: bool = False):
    ...
    def decorator(
        func: Callable[[SessionController, BulinMessageEvent], Awaitable[Any]],
    ):
