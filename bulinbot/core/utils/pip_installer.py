...
from bulinbot.core.utils.bulinbot_path import get_bulinbot_site_packages_path
from bulinbot.core.utils.core_constraints import CoreConstraintsProvider
from bulinbot.core.utils.desktop_core_lock import get_desktop_core_lock_modules
    ...
    try:
        ...
        try:
            ...
            raise ImportError(
                ...
                f"BULINBOT_DESKTOP_CLIENT={os.environ.get('BULINBOT_DESKTOP_CLIENT')})"
            ) from exc
...
def _classify_pip_failure(output_lines: list[str]) -> DependencyConflictError | None:
    ...
    if is_core_conflict:
        message = (
            f"检测到核心依赖版本保护冲突。{detail}插件要求的依赖版本与 BulinBot 核心不兼容，"
...
class PipInstaller:
    def __init__(
        ...
        core_dist_name: str | None = "BulinBot",
    ) -> None:
    ...
    async def install(
        ...
        if is_packaged_desktop_runtime():
            target_site_packages = get_bulinbot_site_packages_path()
