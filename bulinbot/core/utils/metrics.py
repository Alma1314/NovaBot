...
class Metric:
    ...
    def _is_disabled() -> bool:
        """检查是否禁用指标上传（配置或环境变量）"""
        if os.environ.get("BULINBOT_DISABLE_METRICS", "0") == "1":
            return True
        try:
            from bulinbot.core import bulinbot_config

            return bulinbot_config.get("disable_metrics", False)
    ...
    def get_installation_id():
        ...
        config_dir = os.path.join(os.path.expanduser("~"), ".bulinbot")
    ...
    async def upload(**kwargs) -> None:
        """上传相关非敏感的指标以更好地了解 BulinBot 的使用情况。上传的指标不会包含任何有关消息文本、用户信息等敏感信息。
