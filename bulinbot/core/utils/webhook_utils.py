...
from bulinbot.core.config.default import WEBHOOK_SUPPORTED_PLATFORMS


def _get_callback_api_base() -> str:
    try:
        return bulinbot_config.get("callback_api_base", "").rstrip("/")
...
def _get_dashboard_port() -> int:
    try:
        return bulinbot_config.get("dashboard", {}).get("port", 6185)
    except Exception as e:
...
def _is_dashboard_ssl_enabled() -> bool:
    env_ssl = os.environ.get("DASHBOARD_SSL_ENABLE") or os.environ.get(
        "BULINBOT_DASHBOARD_SSL_ENABLE"
    )
    if env_ssl is not None:
        return env_ssl.strip().lower() in {"1", "true", "yes", "on"}

    try:
        return bool(bulinbot_config.get("dashboard", {}).get("ssl", {}).get("enable"))
    except Exception as e:
...
def log_webhook_info(platform_name: str, webhook_uuid: str) -> None:
    ...
    if not callback_base:
        callback_base = "http(s)://<your-bulinbot-domain>"
