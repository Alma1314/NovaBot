...
from bulinbot.core import bulinbot_config, logger
from bulinbot.core.agent.runners.coze.coze_agent_runner import CozeAgentRunner
from bulinbot.core.agent.runners.dashscope.dashscope_agent_runner import (
    DashscopeAgentRunner,
)
from bulinbot.core.agent.runners.deerflow.constants import (
...
from bulinbot.core.agent.runners.dify.dify_agent_runner import DifyAgentRunner
from bulinbot.core.bulin_agent_hooks import MAIN_AGENT_HOOKS
from bulinbot.core.message.components import Image, Record
...
if TYPE_CHECKING:
    from bulinbot.core.agent.runners.base import BaseAgentRunner
    from bulinbot.core.provider.entities import LLMResponse
from bulinbot.core.pipeline.stage import Stage
from bulinbot.core.platform.bulin_message_event import BulinMessageEvent
from bulinbot.core.provider.entities import (
    ProviderRequest,
)
from bulinbot.core.star.star_handler import EventType
from bulinbot.core.utils.config_number import coerce_int_config
from bulinbot.core.utils.metrics import Metric

from .....bulin_agent_context import AgentContextWrapper, BulinAgentContext
...
class ThirdPartyAgentSubStage(Stage):
    async def initialize(self, ctx: PipelineContext) -> None:
        ...
        settings = ctx.bulinbot_config["provider_settings"]
    ...
    async def process(
        self, event: BulinMessageEvent, provider_wake_prefix: str
    ) -> AsyncGenerator[None, None]:
        ...
        self.prov_cfg: dict = next(
            (p for p in bulinbot_config["provider"] if p["id"] == self.prov_id),
            {},
        )
        if not self.prov_id:
        ...
        if self.runner_type == "dify":
            runner = DifyAgentRunner[BulinAgentContext]()
        elif self.runner_type == "coze":
            runner = CozeAgentRunner[BulinAgentContext]()
        elif self.runner_type == "dashscope":
            runner = DashscopeAgentRunner[BulinAgentContext]()
        elif self.runner_type == DEERFLOW_PROVIDER_TYPE:
            runner = DeerFlowAgentRunner[BulinAgentContext]()
        else:
            raise ValueError(
