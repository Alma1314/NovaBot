...
from bulinbot.core.message.components import BaseMessageComponent, ComponentType
from bulinbot.core.message.message_event_result import MessageChain, ResultContentType
...
from bulinbot.core.utils.path_util import path_Mapping
...
class RespondStage(Stage):
    ...
    async def initialize(self, ctx: PipelineContext) -> None:
        self.ctx = ctx
        self.config = ctx.bulinbot_config
        ...
        # 分段回复
        self.enable_seg: bool = ctx.bulinbot_config["platform_settings"][
            "segmented_reply"
        ]["enable"]
        ...
        self.interval_method = ctx.bulinbot_config["platform_settings"][
            "segmented_reply"
        ]["interval_method"]
        self.log_base = float(
            ctx.bulinbot_config["platform_settings"]["segmented_reply"]["log_base"],
        )
        self.interval = [1.5, 3.5]
        if self.enable_seg:
            interval_str: str = ctx.bulinbot_config["platform_settings"][
                "segmented_reply"
            ]["interval"]
    ...
    def is_seg_reply_required(self, event: BulinMessageEvent) -> bool:
        """检查是否需要分段回复"""
