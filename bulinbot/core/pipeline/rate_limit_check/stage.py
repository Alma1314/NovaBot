...
from bulinbot.core.config.bulinbot_config import RateLimitStrategy
...
class RateLimitStage(Stage):
    ...
    async def initialize(self, ctx: PipelineContext) -> None:
        """初始化限流器，根据配置设置限流参数。"""
        self.rate_limit_count = ctx.bulinbot_config["platform_settings"]["rate_limit"][
            "count"
        ]
        self.rate_limit_time = timedelta(
            seconds=ctx.bulinbot_config["platform_settings"]["rate_limit"]["time"],
        )
        self.rl_strategy = ctx.bulinbot_config["platform_settings"]["rate_limit"][
            "strategy"
        ]  # stall or discard
    ...
    async def process(
        ...
        Args:
            event (BulinMessageEvent): 当前消息事件。
