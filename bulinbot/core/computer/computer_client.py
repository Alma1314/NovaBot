...
from bulinbot.core.skills.skill_manager import SANDBOX_SKILLS_ROOT, SkillManager
...
_MANAGED_SKILLS_FILE = ".bulinbot_managed_skills.json"
...
def _collect_sync_skill_dirs() -> list[tuple[str, Path]]:
    ...
    skills_root = Path(get_bulinbot_skills_path())
...
def _discover_bay_credentials(endpoint: str) -> str:
    ...
    # 2. Mono-repo layout: BulinBot/../pkgs/bay/credentials.json
    bulinbot_root = Path(__file__).resolve().parents[3]  # bulinbot/core/computer/ → root
    candidates.append(bulinbot_root.parent / "pkgs" / "bay" / "credentials.json")
