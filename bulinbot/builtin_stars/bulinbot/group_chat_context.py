...
from bulinbot.api.message_components import At, Image, Plain
from bulinbot.api.platform import MessageType
from bulinbot.api.provider import Provider, ProviderRequest
...
class GroupChatContext:
    def __init__(self, acm: BulinBotConfigManager, context: star.Context) -> None:
        self.acm = acm
    ...
    def cfg(self, event: BulinMessageEvent):
    ...
    async def need_active_reply(self, event: BulinMessageEvent) -> bool:
        cfg = self.cfg(event)
    ...
    async def remove_session(self, event: BulinMessageEvent) -> int:
    ...
    async def handle_message(self, event: BulinMessageEvent) -> None:
        if event.get_message_type() != MessageType.GROUP_MESSAGE:
            return
    ...
    async def on_req_llm(self, event: BulinMessageEvent, req: ProviderRequest) -> None:
        umo = event.unified_msg_origin
    ...
    async def _format_message(self, event: BulinMessageEvent, cfg: dict) -> str:
