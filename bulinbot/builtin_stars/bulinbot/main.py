...
from bulinbot.api.message_components import Image, Plain
...
def _iter_message_components(event: BulinMessageEvent):
...
class Main(star.Star):
    def __init__(self, context: star.Context) -> None:
        ...
        try:
            self.group_chat_context = GroupChatContext(
                self.context.bulinbot_config_mgr,
                self.context,
            )
    ...
    async def handle_session_control_agent(self, event: BulinMessageEvent) -> None:
        """会话控制代理"""
    ...
    def group_context_enabled(self, event: BulinMessageEvent):
    ...
    async def decorate_llm_req(
        self, event: BulinMessageEvent, req: ProviderRequest
    ) -> None:
    ...
    async def after_message_sent(self, event: BulinMessageEvent) -> None:
        """消息发送后处理"""
        if self.group_chat_context and self.group_context_enabled(event):
            try:
