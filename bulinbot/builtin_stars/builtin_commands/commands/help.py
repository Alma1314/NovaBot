...
from bulinbot.core.star import command_management
...
class HelpCommand:
    ...
    async def _query_bulinbot_notice(self):
        try:
            async with aiohttp.ClientSession(trust_env=True) as session:
                async with session.get(
                    "https://bulinbot.app/notice.json",
                    timeout=2,
                ) as resp:
    ...
    async def help(self, event: BulinMessageEvent) -> None:
        ...
        notice = ""
        try:
            notice = await self._query_bulinbot_notice()
        except BaseException:
            pass
        ...
        msg_parts = [
            f"BulinBot v{VERSION}(WebUI: {dashboard_version})",
            commands_section,
        ]
        if notice:
