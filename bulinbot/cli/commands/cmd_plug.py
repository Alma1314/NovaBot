...
def _get_data_path() -> Path:
    base = get_bulinbot_root()
    if not check_bulinbot_root(base):
        raise click.ClickException(
            f"{base} is not a valid BulinBot root directory. Use 'bulinbot init' to initialize",
        )
...
def new(name: str) -> None:
    ...
    with open(plug_path / "README.md", "w", encoding="utf-8") as f:
        f.write(
            f"# {name}\n\n{desc}\n\n# Support\n\n[Documentation](https://docs.bulinbot.app)\n"
        )
