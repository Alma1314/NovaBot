...
from ..utils import check_bulinbot_root, get_bulinbot_root
...
def _load_config() -> dict[str, Any]:
    ...
    root = get_bulinbot_root()
    if not check_bulinbot_root(root):
        raise click.ClickException(
            f"{root} is not a valid BulinBot root directory. Use 'bulinbot init' to initialize",
        )
...
def _save_config(config: dict[str, Any]) -> None:
    """Save config file"""
    config_path = get_bulinbot_root() / "data" / "cmd_config.json"
