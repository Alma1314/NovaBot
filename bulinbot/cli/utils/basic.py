...
def check_bulinbot_root(path: str | Path) -> bool:
    """Check if the path is an BulinBot root directory"""
    if not isinstance(path, Path):
        path = Path(path)
    if not path.exists() or not path.is_dir():
        return False
    if not (path / ".bulinbot").exists():
        return False
    return True


def get_bulinbot_root() -> Path:
    """Get the BulinBot root directory path"""
    return Path.cwd()


async def check_dashboard(bulinbot_root: Path) -> None:
    ...
    try:
            ...
            case None:
                ...
                if click.confirm(
                    ...
                    await download_dashboard(
                        ...
                        extract_path=str(bulinbot_root),
                        version=f"v{VERSION}",
                        latest=False,
                    )
        ...
        try:
            await download_dashboard(
                path=str(bulinbot_root / "dashboard.zip"),
                extract_path=str(bulinbot_root),
                version=f"v{VERSION}",
                latest=False,
            )
