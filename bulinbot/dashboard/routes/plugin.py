...
from bulinbot.api import sp
from bulinbot.core import DEMO_MODE, file_token_service, logger
from bulinbot.core.computer.computer_client import sync_skills_to_active_sandboxes
from bulinbot.core.core_lifecycle import BulinBotCoreLifecycle
from bulinbot.core.skills.skill_manager import SkillManager
from bulinbot.core.star.filter.command import CommandFilter
...
from bulinbot.core.star.filter.regex import RegexFilter
...
from bulinbot.core.star.star_manager import (
    PluginManager,
...
class PluginRoute(Route):
    ...
    async def get_plugin_page_bridge_sdk(self):
        ...
        if initial_context:
            ...
            bridge_js += (
                f"\n;window.BulinBotPluginPage?.__setInitialContext({context_json});\n"
            )
        response = cast(
            QuartResponse,
            await make_response(
    ...
    def _apply_plugin_page_security_headers(response: QuartResponse) -> QuartResponse:
        ...
        # When running under the BulinBot Launcher the dashboard is embedded in a
        ...
        if os.environ.get("BULINBOT_LAUNCHER") not in ("1", "true"):
    ...
    async def check_plugin_compatibility(self):
        try:
            ...
            version_spec = data.get("bulinbot_version", "")
            is_valid, message = self.plugin_manager._validate_bulinbot_version_specifier(
                version_spec
            )
            return (
                Response()
    ...
    def _build_registry_source(self, custom_url: str | None) -> RegistrySource:
        """构建注册表源信息"""
        data_dir = get_bulinbot_data_path()
        if custom_url:
            # 对自定义URL生成一个安全的文件名
            ...
            md5_url = "https://api.soulter.top/bulinbot/plugins-md5"
            urls = [
                "https://api.soulter.top/bulinbot/plugins",
                "https://github.com/BulinBotDevs/BulinBot_Plugins_Collection/raw/refs/heads/main/plugin_cache_original.json",
            ]
    ...
    async def get_plugins(self):
        ...
        for plugin, logo_url, pages in results:
            _t = {
                ...
                "bulinbot_version": plugin.bulinbot_version,
    ...
    async def install_plugin(self):
        ...
        try:
            ...
            return {
                ...
                "message": str(e),
                "data": {
                    "warning_type": "bulinbot_version_incompatible",
                    "can_ignore": True,
                },
            }
        except Exception as e:
