...
from bulinbot.core import bulinbot_config, file_token_service, logger
from bulinbot.core.config.bulinbot_config import BulinBotConfig
from bulinbot.core.config.default import (
    CONFIG_METADATA_2,
    CONFIG_METADATA_3,
...
from bulinbot.core.platform.register import platform_cls_map, platform_registry
from bulinbot.core.provider import Provider
from bulinbot.core.provider.register import provider_registry
from bulinbot.core.star.star import StarMetadata, star_registry
from bulinbot.core.utils.bulinbot_path import (
    get_bulinbot_plugin_data_path,
)
...
from bulinbot.core.utils.webhook_utils import ensure_platform_webhook_config
    ...
    def validate(data: dict, metadata: dict = schema, path="") -> None:
        for key, value in data.items():
            ...
            if meta["type"] == "list" and not isinstance(value, list):
                ...
                # 当前仅针对 list[dict] 的情况进行类型校验，以适配 BulinBot 中 platform、provider 的配置
                for item in value:
...
async def _validate_neo_connectivity(
    ...
    if not access_token:
        # Try auto-discovery
        from bulinbot.core.computer.computer_client import _discover_bay_credentials
...
def save_config(
    post_config: dict, config: BulinBotConfig, is_core: bool = False
) -> None:
    """验证并保存配置"""
    errors = None
...
class ConfigRoute(Route):
    def __init__(
        ...
        core_lifecycle: BulinBotCoreLifecycle,
    ) -> None:
        super().__init__(context)
        self.core_lifecycle = core_lifecycle
        self.config: BulinBotConfig = core_lifecycle.bulinbot_config
        self.db = core_lifecycle.db
        self._logo_token_cache = {}  # 缓存logo token，避免重复注册
        self.acm = core_lifecycle.bulinbot_config_mgr
        ...
        self.routes = {
            ...
            "/config/bulinbot/update": ("POST", self.post_bulinbot_configs),
    ...
    async def get_provider_template(self):
        ...
        data = {
            ...
            "providers": bulinbot_config["provider"],
            "provider_sources": bulinbot_config["provider_sources"],
        }
    ...
    async def get_configs(self):
        # plugin_name 为空时返回 BulinBot 配置
        # 否则返回指定 plugin_name 的插件配置
        plugin_name = request.args.get("plugin_name", None)
        if not plugin_name:
            return Response().ok(await self._get_bulinbot_config()).__dict__
    ...
    async def get_embedding_dim(self):
        ...
        try:
            ...
            from bulinbot.core.provider.provider import EmbeddingProvider
            from bulinbot.core.provider.register import provider_cls_map
    ...
    async def post_bulinbot_configs(self):
        ...
        try:
            ...
            await self._save_bulinbot_configs(config, conf_id)
    ...
    def _resolve_config_file_scope(
        self,
    ) -> tuple[str, str, str, StarMetadata, BulinBotConfig]:
        """将请求参数解析为一个明确的配置作用域。
    ...
    async def upload_config_file(self):
        ...
        storage_root_path = Path(get_bulinbot_plugin_data_path()).resolve(strict=False)
    ...
    async def _get_bulinbot_config(self):
        config = self.config
    ...
    async def _save_bulinbot_configs(
        self, post_configs: dict, conf_id: str | None = None
    ) -> None:
        try:
            if conf_id not in self.acm.confs:
                raise ValueError(f"配置文件 {conf_id} 不存在")
            bulinbot_config = self.acm.confs[conf_id]

            # 保留服务端的 t2i_active_template 值
            if "t2i_active_template" in bulinbot_config:
                post_configs["t2i_active_template"] = bulinbot_config[
                    "t2i_active_template"
                ]

            save_config(post_configs, bulinbot_config, is_core=True)
        except Exception as e:
            raise e
