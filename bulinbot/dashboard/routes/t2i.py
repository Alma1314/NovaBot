# bulinbot/dashboard/routes/t2i.py
...
class T2iRoute(Route):
    def __init__(
        self, context: RouteContext, core_lifecycle: BulinBotCoreLifecycle
    ) -> None:
    ...
    async def _reload_all_pipeline_schedulers(self) -> None:
        ...
        for conf_id in self.core_lifecycle.bulinbot_config_mgr.confs:
            await self.core_lifecycle.reload_pipeline_scheduler(conf_id)

    async def _sync_active_template_to_all_configs(self, name: str) -> None:
        """同步当前激活模板到所有配置文件，并热重载对应流水线。"""
        for config in self.core_lifecycle.bulinbot_config_mgr.confs.values():
