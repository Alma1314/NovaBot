...
from bulinbot.core import DEMO_MODE, logger, pip_installer
from bulinbot.core.config.default import VERSION
from bulinbot.core.core_lifecycle import BulinBotCoreLifecycle
from bulinbot.core.db.migration.helper import check_migration_needed_v4, do_migration_v4
from bulinbot.core.updator import BulinBotUpdator
from bulinbot.core.utils.io import download_dashboard, get_dashboard_version
...
class UpdateRoute(Route):
    def __init__(
        ...
        bulinbot_updator: BulinBotUpdator,
        ...
        self.bulinbot_updator = bulinbot_updator
    ...
    async def do_migration(self):
        ...
        try:
            ...
            await do_migration_v4(
                ...
                pim,
                self.core_lifecycle.bulinbot_config,
            )
    ...
    async def check_update(self):
        ...
        try:
            ...
            ret = await self.bulinbot_updator.check_update(None, None, False)
            return Response(
                status="success",
    ...
    async def get_releases(self):
        try:
            ret = await self.bulinbot_updator.get_releases()
    ...
    async def update_project(self):
        ...
        try:
            ...
            await self.bulinbot_updator.update(
                latest=latest,
                version=version,
                proxy=proxy,
                progress_callback=self._make_progress_callback(
                ...
                self.update_progress[progress_id].update(
                    {
                        "status": "success",
                        "stage": "done",
                        "message": "更新成功，BulinBot 将在 2 秒内全量重启以应用新的代码。",
                        "overall_percent": 100,
                    },
                )
                ret = (
                    Response()
                    .ok(None, "更新成功，BulinBot 将在 2 秒内全量重启以应用新的代码。")
                    .__dict__
                )
            ...
            self.update_progress[progress_id].update(
                {
                    "status": "success",
                    "stage": "done",
                    "message": "更新成功，BulinBot 将在下次启动时应用新的代码。",
                    "overall_percent": 100,
                },
            )
            ret = (
                Response()
                .ok(None, "更新成功，BulinBot 将在下次启动时应用新的代码。")
                .__dict__
            )
            return ret, 200, CLEAR_SITE_DATA_HEADERS
        except Exception as e:
