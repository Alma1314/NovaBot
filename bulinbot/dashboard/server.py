...
from bulinbot.core.utils.datetime_utils import to_utc_isoformat
from bulinbot.core.utils.io import (
...
class BulinBotJSONProvider(DefaultJSONProvider):
...
class BulinBotDashboard:
    def __init__(
        ...
        self.config = core_lifecycle.bulinbot_config
        self.db = db

        # Path priority:
        # 1. Explicit webui_dir argument
        # 2. data/dist/ (user-installed / manually updated dashboard)
        # 3. bulinbot/dashboard/dist/ (bundled with the wheel)
        if webui_dir and os.path.exists(webui_dir):
            self.data_path = os.path.abspath(webui_dir)
        else:
            user_dist = os.path.join(get_bulinbot_data_path(), "dist")
        ...
        self.app.json = BulinBotJSONProvider(self.app)
        ...
        self.ur = UpdateRoute(
            self.context,
            core_lifecycle.bulinbot_updator,
            core_lifecycle,
        )
    ...
    async def auth_middleware(self):
        ...
        if (
            os.environ.get("BULINBOT_TEST_MODE") != "true"
    ...
    def _build_dashboard_credentials_display(self) -> str:
        username = self.config["dashboard"].get("username", "bulinbot")
    ...
    def _resolve_dashboard_ssl_config(
        ...
        cert_file = (
            ...
            or os.environ.get("BULINBOT_DASHBOARD_SSL_CERT")
            or ssl_config.get("cert_file", "")
        )
        key_file = (
            os.environ.get("DASHBOARD_SSL_KEY")
            or os.environ.get("BULINBOT_DASHBOARD_SSL_KEY")
            or ssl_config.get("key_file", "")
        )
        ca_certs = (
            os.environ.get("DASHBOARD_SSL_CA_CERTS")
            or os.environ.get("BULINBOT_DASHBOARD_SSL_CA_CERTS")
    ...
    def run(self):
        ip_addr = []
        dashboard_config = self.core_lifecycle.bulinbot_config.get("dashboard", {})
        port = (
            os.environ.get("DASHBOARD_PORT")
            or os.environ.get("BULINBOT_DASHBOARD_PORT")
            or dashboard_config.get("port", 6185)
        )
        host = (
            os.environ.get("DASHBOARD_HOST")
            or os.environ.get("BULINBOT_DASHBOARD_HOST")
        ...
        ssl_enable = _parse_env_bool(
            ...
            or os.environ.get("BULINBOT_DASHBOARD_SSL_ENABLE"),
        ...
        parts = [f"\n ✨✨✨\n  BulinBot v{VERSION} WebUI is ready\n\n"]
    ...
    async def shutdown_trigger(self) -> None:
        ...
        logger.info("BulinBot WebUI 已经被关闭")
