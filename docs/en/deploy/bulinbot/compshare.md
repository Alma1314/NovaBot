...
BulinBot provides an Ollama + BulinBot one-click self-deployment image on Compshare, and also supports Compshare model APIs.

## Use the Ollama + BulinBot One-Click Image
...
2. Open the [BulinBot image page](https://www.compshare.cn/images/0oX7xoGrzfre) and create an instance.
...
After startup, open `http://<instance-public-ip>:6185` in your browser to access the BulinBot dashboard.
...
Use the random password printed in startup logs for first-time login, and use the username shown in the logs (usually `bulinbot`). Change it immediately after login.
...
4. In BulinBot Dashboard -> Providers, edit `ollama_deepseek-r1`, update the model name, and save.

![image](https://files.bulinbot.app/docs/source/images/compshare/image-1.png)

### Use Compshare Model API

BulinBot supports direct access to model APIs provided by Compshare.

1. Find the model you want at [Compshare Model Center](https://console.compshare.cn/light-gpu/model-center).
2. In BulinBot Dashboard -> Providers, click `+ Add Provider`, then choose Compshare.
...
In BulinBot Dashboard, click `Chat` and run `/provider` to view and switch your active provider.
...
You can follow the latest platform integration guides in the [BulinBot Documentation](https://docs.bulinbot.app/en/what-is-bulinbot.html).
...
- More methods: [BulinBot Documentation](https://docs.bulinbot.app/en/what-is-bulinbot.html)

## More Features

For more capabilities, see the [BulinBot Documentation](https://docs.bulinbot.app/en/what-is-bulinbot.html).
