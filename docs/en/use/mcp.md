...
MCP (Model Context Protocol) is a new open standard protocol for establishing secure bidirectional connections between large language models and data sources. Simply put, it extracts function tools as independent services, allowing BulinBot to remotely invoke these function tools via the MCP protocol, which then return results to BulinBot.

![image](https://files.bulinbot.app/docs/source/images/function-calling/image3.png)

BulinBot v3.5.0 supports the MCP protocol, enabling you to add multiple MCP servers and use function tools from MCP servers.
...
For `uv`, you can install it directly via pip. Quick installation via BulinBot WebUI:

![image](https://files.bulinbot.app/docs/en/use/image.png)

Just enter `uv`.

If you're deploying BulinBot with Docker, you can also execute the following command for quick installation:

```bash
docker exec bulinbot python -m pip install uv
```

If you're deploying BulinBot from source, please install it within the created virtual environment.

For `npm`, you need to install `node`.

If you're deploying BulinBot from source or using one-click installation, please refer to [Download Node.js](https://nodejs.org/en/download) to download to your local machine.

If you're using Docker to deploy BulinBot, you need to install `node` in the container (future BulinBot Docker images will include `node` by default). Please execute the following commands:

```bash
sudo docker exec -it bulinbot /bin/bash
...
After installing `node`, you need to restart `BulinBot` to apply the new environment variables.

## Installing MCP Servers

If you're deploying BulinBot with Docker, please install MCP servers in the data directory.
...
Configure it in the BulinBot WebUI:
