...
- `local`: executes on the host machine where BulinBot is running. Use this when the Agent needs local files, command-line tools, or local dependencies.
...
`local` mode mounts Computer Use tools into the host environment where BulinBot runs. The Agent can call the host Shell, host Python, and host filesystem tools.

This means the Agent's boundary is close to the BulinBot process itself. What it can access depends on the system permissions, runtime user, working directory, and operating-system restrictions of the BulinBot process.

### Workspace

In `local` mode, BulinBot prepares a workspace for each session:
...
> [!NOTE]
> The local Python tool executes code through BulinBot's current Python environment. When Python code reads or writes files, use explicit absolute paths or prepare files through filesystem tools in the workspace first.
...
- `Python`: executes Python code in BulinBot's current Python environment.
...
- `Require BulinBot admin permission`
...
- BulinBot temporary directories
- `.bulinbot` under the system temporary directory

If `Require BulinBot admin permission` is disabled, regular users behave much closer to admins for Computer Use tools. Do not disable it unless you understand the risk.
...
`sandbox` mode runs execution actions inside an isolated environment instead of directly on the BulinBot host.

Inside the sandbox, the Agent can still use Shell, Python, and filesystem tools. If the selected sandbox profile supports the `browser` capability, BulinBot also mounts browser automation tools.
...
For sandbox deployment, profiles, TTL, persistence, and browser capabilities, see [Agent Sandbox Environment](/en/use/bulinbot-agent-sandbox).

> [!NOTE]
> Even in `sandbox` mode, `Require BulinBot admin permission` still affects access to Shell, Python, browser, upload/download, and related tools. The exact behavior depends on your configuration.
...
In `sandbox` mode, BulinBot attempts to sync local Skills into the sandbox so the Agent can execute them there.
