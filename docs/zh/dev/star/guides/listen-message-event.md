...
BulinBot 接收消息平台下发的消息，并将其封装为 `BulinMessageEvent` 对象，传递给插件进行处理。

![message-event](https://files.bulinbot.app/docs/zh/dev/star/guides/message-event.svg)

### 消息事件

`BulinMessageEvent` 是 BulinBot 的消息事件对象，其中存储了消息发送者、消息内容等信息。

### 消息对象

`BulinBotMessage` 是 BulinBot 的消息对象，其中存储了消息平台下发的消息具体内容，`BulinMessageEvent` 对象中包含一个 `message_obj` 属性用于获取该消息对象。
...
![message-chain](https://files.bulinbot.app/docs/zh/dev/star/guides/message-chain.svg)
...
在 BulinBot 中，消息链表示为 `List[BaseMessageComponent]` 类型的列表。

## 指令

![message-event-simple-command](https://files.bulinbot.app/docs/zh/dev/star/guides/message-event-simple-command.svg)
...
![command-with-param](https://files.bulinbot.app/docs/zh/dev/star/guides/command-with-param.svg)
...
在 BulinBot 准备调用 LLM 但还未获取会话锁时，会触发 `on_waiting_llm_request` 钩子。
...
async def on_agent_begin(self, event: BulinMessageEvent, run_context: ContextWrapper[BulinAgentContext]):
    print("Agent 开始运行")
```
...
async def on_agent_done(self, event: BulinMessageEvent, run_context: ContextWrapper[BulinAgentContext], resp: LLMResponse):
    print(resp)
```
