...
![image](https://files.bulinbot.app/docs/source/images/plugin/image.png)

5. 点击右下角的 `Create repository`。

### Clone 插件和 NovaBot 项目
...
> [!NOTE]
> NovaBot 插件市场的信息展示依赖于 `metadata.yaml` 文件。

### 调试插件

NovaBot 采用在运行时注入插件的机制。因此，在调试插件时，需要启动 NovaBot 本体。

插件的代码修改后，可以在 NovaBot WebUI 的插件管理处找到自己的插件，点击 `管理`，点击 `重载插件` 即可。
...
3. 在 `__init__` 方法中会传入 `Context` 对象，这个对象包含了 NovaBot 的大多数组件
4. 具体的处理函数 `Handler` 在插件类中定义，如这里的 `helloworld` 函数。
5. 请务必使用 `from nova-bot.api import logger` 来获取日志对象，而不是使用 `logging` 模块。
...
`BulinMessageEvent` 是 NovaBot 的消息事件对象。你可以通过 `BulinMessageEvent` 来获取消息发送者、消息内容等信息。

### NovaBotMessage

`NovaBotMessage` 是 NovaBot 的消息对象。你可以通过 `NovaBotMessage` 来查看消息适配器下发的消息的具体内容。通过 `event.message_obj` 获取。
...
async def on_message(self, event: BulinMessageEvent):
    print(event.message_obj.raw_message) # 平台下发的原始消息在这里
    print(event.message_obj.message) # NovaBot 解析出来的消息链内容
```
...
不是所有的平台都支持所有的消息类型。下方的表格展示了 NovaBot 支持的平台和消息类型的对应关系。
...
![插件 logo 示例](https://files.bulinbot.app/docs/source/images/plugin/plugin_logo.png)
...
事件监听器的注册器在 `nova-bot.api.event.filter` 下，需要先导入。请务必导入，否则会和 python 的高阶函数 filter 冲突。
...
> [!TIP]
> 指令不能带空格，否则 NovaBot 会将其解析到第二个参数。可以使用下面的指令组功能，或者也使用监听器自己解析消息内容。

#### 带参指令

NovaBot 会自动帮你解析指令的参数。

```python
@filter.command("echo")
async def echo(self, event: BulinMessageEvent, message: str):
...
@calc.command("help")
def calc_help(self, event: BulinMessageEvent):
    # /math calc help
...
def help(self, event: BulinMessageEvent):
    yield event.plain_result("这是一个计算器插件，拥有 add, sub 指令。")
```
...
在 NovaBot 默认的执行流程中，在调用 LLM 前，会触发 `on_llm_request` 钩子。
...
async def my_custom_hook_1(self, event: BulinMessageEvent, req: ProviderRequest): # 请注意有三个参数
    print(req) # 打印请求的文本
...
async def on_llm_resp(self, event: BulinMessageEvent, resp: LLMResponse): # 请注意有三个参数
    print(resp)
```
...
async def on_agent_begin(self, event: BulinMessageEvent, run_context: ContextWrapper[BulinAgentContext]): # 请注意有三个参数
    print("Agent 开始运行")
```
...
async def on_agent_done(self, event: BulinMessageEvent, run_context: ContextWrapper[BulinAgentContext], resp: LLMResponse): # 请注意有四个参数
    print(resp)
```
...
@filter.command("test")
async def test(self, event: BulinMessageEvent):
    from nova-bot.api.message_components import Face, Plain
    yield event.chain_result([Face(id=21), Plain("你好呀")])
```

![发送 QQ 表情](https://files.bulinbot.app/docs/source/images/plugin/image-5.png)
...
NovaBot 提供了”强大“的配置解析和可视化功能。能够让用户在管理面板上直接配置插件，而不需要修改代码。

![image](https://files.bulinbot.app/docs/source/images/plugin/QQ_1738149538737.png)
...
- `editor_mode`: 可选。是否启用代码编辑器模式。需要 NovaBot >= `v3.5.10`, 低于这个版本不会报错，但不会生效。默认是 false。
- `editor_language`: 可选。代码编辑器的代码语言，默认为 `json`。
- `editor_theme`: 可选。代码编辑器的主题，可选值有 `vs-light`（默认）， `vs-dark`。
- `_special`: 可选。用于调用 NovaBot 提供的可视化提供商选取、人格选取、知识库选取等功能，详见下文。
...
NovaBot 在载入插件时会检测插件目录下是否有 `_conf_schema.json` 文件，如果有，会自动解析配置并保存在 `data/config/<plugin_name>_config.json` 下（依照 Schema 创建的配置文件实体），并在实例化插件类时传入给 `__init__()`。

```py
from nova-bot.api import NovaBotConfig

class ConfigPlugin(Star):
    def __init__(self, context: Context, config: NovaBotConfig): # NovaBotConfig 继承自 Dict，拥有字典的所有方法
        super().__init__(context)
        self.config = config
        print(self.config)
...
如果您在发布不同版本时更新了 Schema，请注意，NovaBot 会递归检查 Schema 的配置项，如果发现配置文件中缺失了某个配置项，会自动添加默认值。但是 NovaBot 不会删除配置文件中**多余的**配置项，即使这个配置项在新的 Schema 中不存在（您在新的 Schema 中删除了这个配置项）。
...
函数工具给了大语言模型调用外部工具的能力。在 NovaBot 中，函数工具有多种定义方式。
...
```py
from nova-bot.api import FunctionTool
from nova-bot.api.event import BulinMessageEvent
...
这个形式定义的工具函数会被自动加载到 NovaBot Core 中，在 Core 请求大模型时会被自动带上。

请务必按照以下格式编写一个工具（包括**函数注释**，NovaBot 会解析该函数注释，请务必将注释格式写对）
...
>
> ```py
> func_tool = self.context.get_llm_tool_manager() # 获取 NovaBot 的 LLM Tool Manager，包含了所有插件和 MCP 注册的 Tool
...
```py
from nova-bot.core.conversation_mgr import Conversation
...
`PersonaManager` 负责统一加载、缓存并提供所有人格（Persona）的增删改查接口，同时兼容 NovaBot 4.x 之前的旧版人格格式（v3）。  
...
async def test_(self, event: BulinMessageEvent):
    from nova-bot.api.platform import AiocqhttpAdapter # 其他平台同理
