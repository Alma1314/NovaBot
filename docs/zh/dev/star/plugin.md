...
### Clone 插件和 BulinBot 项目

Clone BulinBot 项目本体和刚刚创建的插件仓库到本地。
...
> [!NOTE]
> BulinBot 插件市场的信息展示依赖于 `metadata.yaml` 文件。

### 调试插件

BulinBot 采用在运行时注入插件的机制。因此，在调试插件时，需要启动 BulinBot 本体。

插件的代码修改后，可以在 BulinBot WebUI 的插件管理处找到自己的插件，点击 `管理`，点击 `重载插件` 即可。
...
3. 在 `__init__` 方法中会传入 `Context` 对象，这个对象包含了 BulinBot 的大多数组件
4. 具体的处理函数 `Handler` 在插件类中定义，如这里的 `helloworld` 函数。
5. 请务必使用 `from bulinbot.api import logger` 来获取日志对象，而不是使用 `logging` 模块。
...
`BulinMessageEvent` 是 BulinBot 的消息事件对象。你可以通过 `BulinMessageEvent` 来获取消息发送者、消息内容等信息。

### BulinBotMessage

`BulinBotMessage` 是 BulinBot 的消息对象。你可以通过 `BulinBotMessage` 来查看消息适配器下发的消息的具体内容。通过 `event.message_obj` 获取。
...
async def on_message(self, event: BulinMessageEvent):
    ...
    print(event.message_obj.message) # BulinBot 解析出来的消息链内容
```
...
不是所有的平台都支持所有的消息类型。下方的表格展示了 BulinBot 支持的平台和消息类型的对应关系。
...
事件监听器的注册器在 `bulinbot.api.event.filter` 下，需要先导入。请务必导入，否则会和 python 的高阶函数 filter 冲突。
...
> [!TIP]
> 指令不能带空格，否则 BulinBot 会将其解析到第二个参数。可以使用下面的指令组功能，或者也使用监听器自己解析消息内容。

#### 带参指令

BulinBot 会自动帮你解析指令的参数。
...
async def on_bulinbot_loaded(self):
    print("BulinBot 初始化完成")

```

##### LLM 请求时

在 BulinBot 默认的执行流程中，在调用 LLM 前，会触发 `on_llm_request` 钩子。
...
@filter.command("test")
async def test(self, event: BulinMessageEvent):
    from bulinbot.api.message_components import Face, Plain
...
BulinBot 提供了”强大“的配置解析和可视化功能。能够让用户在管理面板上直接配置插件，而不需要修改代码。
...
- `editor_mode`: 可选。是否启用代码编辑器模式。需要 BulinBot >= `v3.5.10`, 低于这个版本不会报错，但不会生效。默认是 false。
- `editor_language`: 可选。代码编辑器的代码语言，默认为 `json`。
- `editor_theme`: 可选。代码编辑器的主题，可选值有 `vs-light`（默认）， `vs-dark`。
- `_special`: 可选。用于调用 BulinBot 提供的可视化提供商选取、人格选取、知识库选取等功能，详见下文。
...
BulinBot 在载入插件时会检测插件目录下是否有 `_conf_schema.json` 文件，如果有，会自动解析配置并保存在 `data/config/<plugin_name>_config.json` 下（依照 Schema 创建的配置文件实体），并在实例化插件类时传入给 `__init__()`。

```py
from bulinbot.api import BulinBotConfig

class ConfigPlugin(Star):
    def __init__(self, context: Context, config: BulinBotConfig): # BulinBotConfig 继承自 Dict，拥有字典的所有方法
        super().__init__(context)
        self.config = config
        print(self.config)
...
如果您在发布不同版本时更新了 Schema，请注意，BulinBot 会递归检查 Schema 的配置项，如果发现配置文件中缺失了某个配置项，会自动添加默认值。但是 BulinBot 不会删除配置文件中**多余的**配置项，即使这个配置项在新的 Schema 中不存在（您在新的 Schema 中删除了这个配置项）。
...
函数工具给了大语言模型调用外部工具的能力。在 BulinBot 中，函数工具有多种定义方式。
...
```py
from bulinbot.api import FunctionTool
from bulinbot.api.event import BulinMessageEvent
...
要将上述工具注册到 BulinBot，可以在插件主文件的 `__init__.py` 中添加以下代码：
...
这个形式定义的工具函数会被自动加载到 BulinBot Core 中，在 Core 请求大模型时会被自动带上。

请务必按照以下格式编写一个工具（包括**函数注释**，BulinBot 会解析该函数注释，请务必将注释格式写对）
...
>
> ```py
> func_tool = self.context.get_llm_tool_manager() # 获取 BulinBot 的 LLM Tool Manager，包含了所有插件和 MCP 注册的 Tool
...
v4.0.0 后，BulinBot 支持会话粒度的多配置文件。
...
async def test_(self, event: BulinMessageEvent):
    from bulinbot.api.platform import AiocqhttpAdapter # 其他平台同理
