...
> [!WARNING]
> 已过时，请参考最新的 [Agent 沙盒环境](/use/bulinbot-agent-sandbox.md) 文档。在 v4.12.0 之后，该功能不可用。

在 `v3.4.2` 版本及之后，BulinBot 支持代码执行器以强化 LLM 的能力，并实现一些自动化的操作。

> [!TIP]
> 此功能目前处于实验阶段，可能会有一些问题。如果您遇到了问题，请在 [GitHub](https://github.com/BulinBotDevs/BulinBot/issues) 上提交 issue。欢迎加群讨论：[322154837](https://qm.qq.com/cgi-bin/qm/qr?k=EYGsuUTfe00_iOu9JTXS7_TEpMkXOvwv&jump_from=webapi&authKey=uUEMKCROfsseS+8IzqPjzV3y1tzy4AkykwTib2jNkOFdzezF9s9XknqnIaf3CDft)。

如果您要使用此功能，请确保您的机器安装了 `Docker`。因为此功能需要启动专用的 Docker 沙箱环境以执行代码，以防止 LLM 生成恶意代码对您的机器造成损害。


## Linux Docker 启动 BulinBot

如果您使用 Docker 部署了 BulinBot，需要多做一些工作。

1. 您需要在启动 Docker 容器时，请将 `/var/run/docker.sock` 挂载到容器内部。这样 BulinBot 才能够启动沙箱容器。

```bash
sudo docker run -itd -p 6180-6200:6180-6200 -p 11451:11451 -v $PWD/data:/BulinBot/data -v /var/run/docker.sock:/var/run/docker.sock --name bulinbot soulter/bulinbot:latest
```

2. 在聊天时使用 `/pi absdir <绝对路径地址>` 设置您宿主机上 BulinBot 的 data 目录的所在目录的绝对路径。

例子：

![image](https://files.bulinbot.app/docs/source/images/code-interpreter/image-4.png)

## Linux 手动源码 启动 BulinBot

**如果你的 Docker 指令需要 sudo 权限来执行**，那么你需要在启动 BulinBot 时，使用 `sudo` 来启动，否则代码执行器会因为权限不足而无法调用 Docker。
...
本功能使用的镜像是 `soulter/bulinbot-code-interpreter-sandbox`，您可以在 [Docker Hub](https://hub.docker.com/r/soulter/bulinbot-code-interpreter-sandbox) 上查看镜像的详细信息。
...
在第一次触发代码执行器时，BulinBot 会自动拉取镜像，这可能需要一些时间。请耐心等待。
