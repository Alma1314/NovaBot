...
电脑能力（Computer Use）决定 Agent 是否可以在 BulinBot 运行环境中执行代码、访问文件、调用 Shell。
...
- `local`：在 BulinBot 所在机器上执行，适合需要访问本机文件、命令行工具或本地依赖的场景。
- `sandbox`：在隔离沙盒中执行，适合希望降低本机风险、或让多用户使用自动化能力的场景。

## Local 模式

`local` 模式会把电脑能力挂载到 BulinBot 所在的主机环境。Agent 可以调用本机 Shell、本机 Python，以及本机文件系统工具。

这意味着 Agent 的能力边界接近 BulinBot 进程本身：它能访问什么，取决于 BulinBot 进程的系统权限、运行用户、工作目录和操作系统限制。

### Workspace

在 `local` 模式下，BulinBot 会为每个会话准备一个 workspace：
...
> [!NOTE]
> 本地 Python 工具会调用 BulinBot 当前 Python 环境执行代码。编写会读写文件的 Python 代码时，建议使用明确的绝对路径，或先通过文件工具在 workspace 中准备文件。
...
- `Python`：使用 BulinBot 当前 Python 环境执行 Python 代码。
...
关闭“需要 BulinBot 管理员权限”后，普通用户在电脑能力工具上的行为会接近管理员。除非你非常清楚风险，否则不建议关闭。
...
`sandbox` 模式会把执行动作放到隔离环境中，而不是直接在 BulinBot 主机上运行。
...
- `Shipyard Neo`：BulinBot 推荐的远程/独立部署沙盒服务，适合长期运行和多人使用。
...
使用 `CUA` 时，工作目录和可用命令取决于所选 CUA image 与运行方式。Linux CUA 容器通常提供类 Unix Shell；Windows、Android 等非 POSIX 镜像不保证支持 `sh`、`ls`、`rm`、`base64` 等命令，BulinBot 会对部分 shell fallback 操作返回明确错误。

沙盒部署、驱动器选择、CUA 配置、profile、TTL、数据持久化、浏览器能力等内容请参考：[Agent 沙盒环境](/use/bulinbot-agent-sandbox)。

> [!NOTE]
> 即使在 `sandbox` 模式下，“需要 BulinBot 管理员权限”仍会影响 Shell、Python、浏览器、上传下载等工具的调用权限。具体权限取决于你的配置。
...
在 `sandbox` 模式下，BulinBot 会尝试把本地 Skills 同步到沙盒中，让 Agent 在沙盒内按 Skill 指令执行。
