...
NovaBot 的指令通过插件机制注册。为了保持主程序轻量，当前只有少量基础指令随 NovaBot 主程序内置加载；更多管理类、扩展类指令已经迁移到独立插件中维护。
...
以下指令由 NovaBot 主程序自带，默认随 NovaBot 加载：

- `/help`：查看当前启用的指令和 NovaBot 版本信息。
...
- `/new`：创建并切换到一个新对话。
- `/dashboard_update`：更新 NovaBot WebUI。该指令需要管理员权限。
...
- `UID`：当前发送者的用户 ID。它通常用于添加 NovaBot 管理员。
...
原始 UMO 往往比较长，也不一定能直接看出它对应哪个群或哪个用户。设置 `/name` 后，NovaBot 会在 WebUI 的 UMO 列表、会话来源选择、定时任务投递目标、对话数据等位置优先展示这个别名，帮助管理员更快识别和选择目标会话，降低配置路由规则、Cron 投递目标或会话规则时选错来源的概率。
...
如果当前会话没有正在运行的任务，NovaBot 会提示当前会话没有运行中的任务。
...
部分指令需要 NovaBot 管理员权限，例如 `/dashboard_update`、`/name`、`/op`、`/deop`、`/provider`、`/model`、`/persona` 等。
