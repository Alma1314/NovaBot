# 使用 Docker 部署 BulinBot

> [!WARNING]
> 通过 Docker 可以方便地将 BulinBot 部署到 Windows, Mac, Linux 上。
>
> 以下教程默认您的环境已安装 Docker。如果没有安装，请参考 [Docker 官方文档](https://docs.docker.com/get-docker/) 进行安装。

## 通过 Docker Compose 部署

::: details 只部署 BulinBot（通用方式）

首先，需要 Clone BulinBot 仓库到本地：
...
配置和使用详见 [Agent 沙盒环境](/use/bulinbot-agent-sandbox.md) 文档。
:::

::: details 和 NapCat 一起部署

如果您想对接 NapCat，使用这种方式可以同时部署 BulinBot 和 NapCat。

```bash
mkdir bulinbot
cd bulinbot
wget https://raw.githubusercontent.com/NapNeko/NapCat-Docker/main/compose/bulinbot.yml
sudo docker compose -f bulinbot.yml up -d
```
