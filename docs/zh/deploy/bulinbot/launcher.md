# 使用 NovaBot 启动器部署 NovaBot

## NovaBot 一键启动器

NovaBot 一键启动器支持 Windows、MacOS、Linux 等多端部署。

0. 打开 [Alma1314/nova-bot-launcher](https://github.com/Alma1314/nova-bot-launcher)
1.  **(可选但推荐)** 给本项目点个 [**Star ⭐**](https://github.com/Alma1314/nova-bot-launcher)，你的支持是作者更新和维护的动力！
2. 找到右边的 Releases，点击最新版本的 Release，在新的页面的 Assets 中下载对应你系统的安装器。

如，Windows X86 的用户应该下载 `NovaBot.Launcher_0.2.1_x64-setup.exe`，Windows on Arm 的用户应该下载 `NovaBot.Launcher_0.2.1_arm64-setup.exe`，MacOS M 芯片的用户下载 `NovaBot.Launcher_0.2.1_aarch64.dmg`。
...
推荐使用上面提到的 NovaBot 一键启动器来部署 NovaBot，因为它更简单、更自动化、更现代化，适合大多数用户。
...
打开 https://github.com/Alma1314/NovaBotLauncher/releases/latest 
...
将 `launcher_nova-bot_en.bat` 批处理文件拖进去回车运行。

> [!WARNING]
> - 这个脚本没有病毒。如果提示 `Windows 已保护您的电脑`，请点击 `更多信息`，然后点击 `仍要运行`。
>
> - 脚本默认使用 `python` 指令来执行代码，如果你想指定 Python 解释器器路径或者指令，请修改 `launcher_nova-bot_en.bat` 文件。找到 `set PYTHON_CMD=python` 这一行，将 `python` 改为你的 Python 解释器路径或指令。
>

如果没有检测到 Python 环境，脚本将会提示并退出。

脚本将自动检测目录下是否有 `NovaBot` 文件夹，如果没有，将会从 [GitHub](https://github.com/Alma1314/NovaBot/releases/latest) 自动下载最新的 NovaBot 源码。下载好后，会自动安装 NovaBot 的依赖并运行。
...
>
> **当管理面板打开时遇到 404 错误：**
> 在 [release](https://github.com/Alma1314/NovaBot/releases) 页面下载dist.zip，解压拖到 NovaBot/data 下。还不行请重启电脑（来自群里的反馈）
...
回到 `launcher_nova-bot_en.bat` 文件，右键点击 `在记事本中编辑`，找到 `set PYTHON_CMD=python` 这一行，将 `python` 改为你的 Python 解释器路径或指令，路径两端的双引号不要删。
