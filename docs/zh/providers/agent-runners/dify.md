...
![image](https://files.bulinbot.app/docs/source/images/dify/image.png)
...
>[!TIP]
>请确保你在 BulinBot 里设置的 APP 类型和 Dify 里面创建的应用的类型一致。
...
![image](https://files.bulinbot.app/docs/source/images/dify/chat-agent-api-key.png)

![image](https://files.bulinbot.app/docs/source/images/dify/chat-agent-api-key-2.png)
...
对于 Workflow 应用，BulinBot 在每次请求时会附上两个变量:

- `bulinbot_text_query`: 输入变量名。即用户输入的文本内容。
- `bulinbot_session_id`: 会话 ID

你可以在配置中自定义输入变量名，即上图配置中的 “Prompt 输入变量名”。

您需要修改您的 Workflow 的输入的变量名以适配 BulinBot 的输入。

最终，Workflow 会输出一个结果，您可以自定义这个结果的变量名，即上图配置中的 “Dify Workflow 输出变量名”，默认为  `bulinbot_wf_output`。你需要在 Dify 的 Workflow 的输出节点中配置这个变量名，否则 BulinBot 无法正确解析。
...
![image](https://files.bulinbot.app/docs/source/images/dify/workflow-api-key.png)
...
当设置变量后，BulinBot 会在下次向 Dify 请求时附上您设置的变量，以灵活适配您的 Workflow。
